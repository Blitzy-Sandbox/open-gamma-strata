...
final class ResultMatchersSpec
  ...
  // ---------------------------------------------------------------------------
  // Each matcher in its failing direction, on every outcome shape.
  //
  // The cases above read the sentences of the five matchers over the value and
  // accumulating channels and over both shapes a partial result can fail in.
  // Two things they leave: the validating channel, which is read directly only
  // through its instance, and the sentence the framework *displays* - a
  // diagnostic inspected on a `MatchResult` is the text a matcher offers, while
  // the text a spec author reads is the one the framework selects from it, and
  // only a provoked assertion shows which of the two it picked. The five cases
  // below close both, so that each matcher is asserted on an outcome it must
  // reject in every shape that outcome can take, with the rejection sentence
  // written out as a literal.
  // ---------------------------------------------------------------------------

  test("beSuccess accepts the successful form of all four shapes and reports the failures of every other form") {
    // One failure with one reason is used in every shape, so the sentence expected below is
    // the same literal throughout and any shape that rendered differently stands out.
    val accepted: List[MatchResult] = List(
      beSuccess.matcher[FailureOr[Int]].apply(singleSuccess(Answer)),
      beSuccess.matcher[ResultNec[Int]].apply(chainSuccess(Answer)),
      beSuccess.matcher[ValidatedFailures[Int]].apply(validatedSuccess(Answer)),
      beSuccess.matcher[ValueWithFailures[Int]].apply(valueOnly(Answer))
    )
    accepted should have size 4
    ...
    // The five failing shapes: a single failure, a chain, the validating form of that chain,
    // and the two shapes a partial result fails in - failures alone, and failures with a
    // value, which is the one that would pass a matcher deciding from the value.
    val rejected: List[MatchResult] = List(
      beSuccess.matcher[FailureOr[Int]].apply(singleFailure(InvalidFailure)),
      beSuccess.matcher[ResultNec[Int]].apply(chainFailure(NonEmptyChain.one(InvalidFailure))),
      beSuccess.matcher[ValidatedFailures[Int]].apply(validatedFailure(NonEmptyChain.one(InvalidFailure))),
      beSuccess.matcher[ValueWithFailures[Int]].apply(failuresOnly(NonEmptyChain.one(InvalidFailure))),
      beSuccess.matcher[ValueWithFailures[Int]].apply(partial(NonEmptyChain.one(InvalidFailure), Answer))
    )
    ...
    // The ordinary assertion form over the validating channel, which no case above writes:
    // the instance of that channel is reached through `should` and not only directly.
    validatedSuccess(Answer) should beSuccess
    validatedFailure(NonEmptyChain.one(InvalidFailure)) shouldNot beSuccess
  }

  test("haveValue accepts the value of all four shapes and separates a mismatch from a failure on each") {
    singleSuccess(Answer) should haveValue(Answer)
    chainSuccess(Answer) should haveValue(Answer)
    validatedSuccess(Answer) should haveValue(Answer)
    valueOnly(Answer) should haveValue(Answer)

    // A success carrying another value is a mismatch in every shape, and the sentence names
    // the value asked for and the value found.
    val mismatched: List[MatchResult] = List(
      haveValue(Answer + 1).matcher[FailureOr[Int]].apply(singleSuccess(Answer)),
      haveValue(Answer + 1).matcher[ResultNec[Int]].apply(chainSuccess(Answer)),
      haveValue(Answer + 1).matcher[ValidatedFailures[Int]].apply(validatedSuccess(Answer)),
      haveValue(Answer + 1).matcher[ValueWithFailures[Int]].apply(valueOnly(Answer))
    )
    mismatched should have size 4
    mismatched.foreach { result =>
      result.matches shouldBe false
      result.failureMessage shouldBe s"Expected Success with value: <8> but was: <$Answer>"
      ()
    }

    // An outcome holding failures takes the other branch of the sentence in every shape,
    // including the partial success that holds the very value asked for.
    val failed: List[MatchResult] = List(
      haveValue(Answer).matcher[FailureOr[Int]].apply(singleFailure(InvalidFailure)),
      haveValue(Answer).matcher[ResultNec[Int]].apply(chainFailure(NonEmptyChain.one(InvalidFailure))),
      haveValue(Answer).matcher[ValidatedFailures[Int]].apply(validatedFailure(NonEmptyChain.one(InvalidFailure))),
      haveValue(Answer).matcher[ValueWithFailures[Int]].apply(failuresOnly(NonEmptyChain.one(InvalidFailure))),
      haveValue(Answer).matcher[ValueWithFailures[Int]].apply(partial(NonEmptyChain.one(InvalidFailure), Answer))
    )
    failed should have size 5
    ...
    validatedFailure(NonEmptyChain.one(InvalidFailure)) shouldNot haveValue(Answer)
    validatedSuccess(Answer) shouldNot haveValue(Answer + 1)
  }

  test("the three failure matchers report the validating and partial shapes with the sentences of the other two") {
    val invalid: ValidatedFailures[Int] = validatedFailure(NonEmptyChain.one(InvalidFailure))
    val valid: ValidatedFailures[Int] = validatedSuccess(Answer)
    val both: ValueWithFailures[Int] = partial(NonEmptyChain.one(InvalidFailure), Answer)
    val onlyValue: ValueWithFailures[Int] = valueOnly(Answer)

    // The accepting direction over the validating channel, which the cases above assert only
    // for `beFailureWith`.
    invalid should beFailure
    invalid should beFailureWith(FailureReason.INVALID)
    invalid should haveFailureMessageMatching("Schedule is invalid")
    valid shouldNot beFailure
    invalid shouldNot beFailureWith(FailureReason.PARSING)
    // The match is against the whole message here too, so a pattern naming only part of it
    // does not match.
    invalid shouldNot haveFailureMessageMatching("Schedule")

    // The rejecting direction, sentence by sentence: a success is reported as a success
    // rather than as a mismatch, the failure test coming first in both matchers that take an
    // argument.
    beFailure.matcher[ValidatedFailures[Int]].apply(valid).failureMessage shouldBe
      s"Expected Failure but was Success with value: <$Answer>"
    beFailure.matcher[ValidatedFailures[Int]].apply(invalid).negatedFailureMessage shouldBe
      s"Expected Success but was Failure with $InvalidPhrase"
    beFailureWith(FailureReason.PARSING).matcher[ValidatedFailures[Int]].apply(valid).failureMessage shouldBe
      s"Expected Failure with reason: <PARSING> but was Success with value: <$Answer>"
    beFailureWith(FailureReason.PARSING).matcher[ValidatedFailures[Int]].apply(invalid).failureMessage shouldBe
      s"Expected Failure with reason: <PARSING> but was Failure with $InvalidPhrase"
    haveFailureMessageMatching("Schedule").matcher[ValidatedFailures[Int]].apply(valid).failureMessage shouldBe
      s"Expected Failure with message matching: <Schedule> but was Success with value: <$Answer>"
    haveFailureMessageMatching("Schedule").matcher[ValidatedFailures[Int]].apply(invalid).failureMessage shouldBe
      "Expected Failure with message matching: <Schedule> but was Failure with message: <Schedule is invalid>"

    // The partial shapes report the same sentences: a value alone is a success to all three
    // matchers, and a value accompanied by failures is described by its failures.
    beFailure.matcher[ValueWithFailures[Int]].apply(onlyValue).failureMessage shouldBe
      s"Expected Failure but was Success with value: <$Answer>"
    beFailureWith(FailureReason.PARSING).matcher[ValueWithFailures[Int]].apply(onlyValue).failureMessage shouldBe
      s"Expected Failure with reason: <PARSING> but was Success with value: <$Answer>"
    beFailureWith(FailureReason.PARSING).matcher[ValueWithFailures[Int]].apply(both).failureMessage shouldBe
      s"Expected Failure with reason: <PARSING> but was Failure with $InvalidPhrase"
    haveFailureMessageMatching("Schedule").matcher[ValueWithFailures[Int]].apply(onlyValue).failureMessage shouldBe
      s"Expected Failure with message matching: <Schedule> but was Success with value: <$Answer>"
    haveFailureMessageMatching("Schedule").matcher[ValueWithFailures[Int]].apply(both).failureMessage shouldBe
      "Expected Failure with message matching: <Schedule> but was Failure with message: <Schedule is invalid>"
  }

  test("a failed assertion of each of the five matchers displays that matcher's own sentence") {
    val success: FailureOr[Int] = singleSuccess(Answer)
    val invalid: ValidatedFailures[Int] = validatedFailure(NonEmptyChain.one(InvalidFailure))

    // Each assertion below is provoked deliberately and its report caught, because the text
    // a spec author reads is the text the framework selected - not every sentence a matcher
    // carries is displayed, and a matcher that offered the right sentence on the wrong side
    // would still misdescribe the outcome in the log.
    intercept[TestFailedException](invalid should beSuccess).getMessage shouldBe
      s"Expected Success but was Failure with $InvalidPhrase"
    intercept[TestFailedException](success should haveValue(Answer + 1)).getMessage shouldBe
      s"Expected Success with value: <8> but was: <$Answer>"
    intercept[TestFailedException](invalid should haveValue(Answer)).getMessage shouldBe
      s"Expected Success but was Failure with $InvalidPhrase"
    intercept[TestFailedException](success should beFailure).getMessage shouldBe
      s"Expected Failure but was Success with value: <$Answer>"
    intercept[TestFailedException](invalid should beFailureWith(FailureReason.PARSING)).getMessage shouldBe
      s"Expected Failure with reason: <PARSING> but was Failure with $InvalidPhrase"
    intercept[TestFailedException](invalid should haveFailureMessageMatching("Schedule")).getMessage shouldBe
      "Expected Failure with message matching: <Schedule> but was Failure with message: <Schedule is invalid>"
  }

  test("a failed negated assertion of each of the five matchers displays that matcher's negated sentence") {
    val success: FailureOr[Int] = singleSuccess(Answer)
    val invalid: ValidatedFailures[Int] = validatedFailure(NonEmptyChain.one(InvalidFailure))

    // The negated sentence of every matcher is displayed only by a failing `shouldNot`, and
    // the consuming suites write `shouldNot` far more often than they see it fail, so this
    // is the case that keeps those five sentences honest.
    intercept[TestFailedException](success shouldNot beSuccess).getMessage shouldBe
      s"Expected Failure but was Success with value: <$Answer>"
    intercept[TestFailedException](success shouldNot haveValue(Answer)).getMessage shouldBe
      s"Expected Success with a value other than <$Answer> but was: <$Answer>"
    intercept[TestFailedException](invalid shouldNot beFailure).getMessage shouldBe
      s"Expected Success but was Failure with $InvalidPhrase"
    intercept[TestFailedException](invalid shouldNot beFailureWith(FailureReason.INVALID)).getMessage shouldBe
      s"Expected Failure with a reason other than <INVALID> but was Failure with $InvalidPhrase"
    intercept[TestFailedException](
      invalid shouldNot haveFailureMessageMatching("Schedule is invalid")).getMessage shouldBe
      "Expected Failure with no message matching: <Schedule is invalid> but was Failure with " +
...
// why none of the 32 cases above maps to a Java test method. They divide as:
//
...
//   - 3 cases stating the contract of each matcher as a table over all four
//     outcome shapes, which is where the validating channel is asserted
//     through the matchers rather than only through its instance, and where
//     every shape a matcher must reject carries its rejection sentence as a
//     literal;
//   - 2 cases reading the sentence the framework actually displays - for a
//     failed assertion and for a failed negated assertion of each of the five
//     matchers - which is the only way to observe which of the two sentences
//     a matcher carries was selected;
...
//
// All 32 count towards the test total this module is required to reach.
// ---------------------------------------------------------------------------
