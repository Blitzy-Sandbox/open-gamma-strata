 ...
 * sub-array boundary beyond the end of an array, an array of matrix rows that differ in
 * length, or the smallest element of an array that has none - is checked here, before
 * anything is allocated or read, and is therefore reported as this object's exception. A
 * violation of ''index'' - a single element addressed outside an array - is left to the array
 * access itself, which raises the index exception of the runtime: a check in front of it
 * would change the exception a caller sees for no gain, since the access reports the same
 * fault at the same instant, and the Java original reported it the same way. `DoubleArray`
 * and `DoubleMatrix` record which of their members falls on which side.
 *
