...
| 16 | Text loading | `ResourceLocator` decodes leniently, substituting a replacement character for malformed input, and reads a resource of any size; `ofFile`/`ofPath` issue the open whatever kind of source the path names, so a device is read to its end and a directory fails in the open itself | `io.Resources` decodes UTF-8 **strictly** — malformed or unmappable input fails the effect with an `IOException` naming the source — and refuses a source larger than its `MaxBytes` ceiling (64 MiB), which is private to `Resources` and met only as the failure that names it. `readFileText` also opens a **regular file** only: the path is described before it is opened (`Files.readAttributes`) and a directory, a named pipe, a socket or a device is refused *before* the open, with an `IOException` naming the source and saying what the platform described it as (measured on a warm JVM: a FIFO with no writer refused in 1 ms, a directory in 1 ms, `/dev/urandom` and `/dev/zero` in under a millisecond). Everything the Java `ResourceLocator` could read that stats as a regular file still reads, symbolic links being followed — measured: an ordinary file, a symlink to one, and `/proc/self/environ`, which the platform describes as regular. The classpath reader applies the same judgement to a location it resolved on the file system, keeping its existing directory wording. A read that stops making progress is bounded too — row 50. `readFileText` is an unconfined filesystem reader, exactly as the Java original was: its only callers are in test scope — `ResourcesSpec` — and **callers must not pass it an untrusted path** | A substituted character inside a captured baseline is a silently altered expectation: the measurement still runs, against a value nobody captured. And the kind of a source decides whether its `open` can be waited out at all: the open of a named pipe with no writer, of a socket, of some devices is defined to wait, and that wait is in a native call no thread interrupt aborts, so a reader of caller-supplied paths that opens first cannot recover from it afterwards (CWE-400/CWE-410) |
...
| 25 | `ReferenceData` store | `ImmutableReferenceData.getValues()` publishes the erased `Map<ReferenceDataId<?>, Object>`; each entry's type is checked reflectively at insertion | No `values` accessor exists, and there is no raw-map factory: a store is read through `findValue`/`getValue`/`containsValue`, and built from `Entry[T]`, which binds value type to id type at compile time. **No cast remains at lookup either**: `findValue` narrows the value it found with the `valueType` witness the asking identifier carries, so a value the identifier does not recognise is reported as an absence rather than returned mistyped (row 53 and (c)-53 state that member and what an implementation supplies) | Removing the reflective per-entry check (Rule 6, D-5) is only sound if the erased store is not published — and the check is not lost with it, but moved to the lookup, where the identifier applies its own witness |
...
| 49 | Bounded parse text | No text-bearing parse has a length ceiling. `StandardId.of` states the permitted lengths of its two parts as `1, Integer.MAX_VALUE` (`StandardId.java:165-166`); `CurrencyAmount.parse` tests only a minimum (`CurrencyAmount.java:120`); `FxRate.parse`, `Tenor.parse`, `MarketTenor.parse` and `Frequency.parse` test none. The scanning, case folding and copying each performs is therefore proportional to text whose length the sender chose | Each of those states a ceiling and tests it **first**, before the text is folded, copied or scanned: 65 536 characters per `StandardId` part and 131 073 for its whole text, and 256 for the amount text of `CurrencyAmount`, the rate text of `FxRate` and the text of `Tenor`, `MarketTenor` and `Frequency` — one figure, the one `Money`, `BigMoney` and `Decimal` already applied to the same numeral. Text past the bound is reported with the wording of a value that could not be read rather than with a wording of its own. See (c)-49 below, which also lists the three figures in this area that are **Java's own**, restated rather than added | Refusing a name should cost what the name costs: without a ceiling, the work of a refusal is proportional to text arriving from outside the library and chosen by its sender (CWE-400/CWE-770). Each figure is derived from what the grammar can express rather than from what a machine can hold, which is what keeps it a bound on work rather than on meaning |
| 50 | Bounded reading time | `ResourceLocator` reads until the source ends. A source that never yields — a named pipe, a device, a stream whose writer has stopped without closing — holds a thread and a file handle for as long as it chooses; no Java source carries a time bound | `io.Resources` abandons a read that has not completed within its `ReadTimeLimit` of two minutes, applied with `timeoutAndForget` so that the **caller** is not made to wait for a read that will not finish, and fails the effect with an `IOException` naming the source and the bound; the handle of the abandoned read still comes back, because release runs for a cancelled use and a cancelled acquisition alike. What the bound does **not** do is recover the thread of a read stalled in a call the platform will not interrupt, so thread retention is answered at its root by row 16's pre-open refusal rather than here: neither reader issues an open against a source the platform has just described as something other than a regular file (measured: reading a FIFO with no writer fails in about a millisecond on a warm JVM - and in a few hundred on the first read of a process, while the classes of the read are still being loaded - and no thread of the process is left in `open0`, where opening it first left one `RUNNABLE` there for the life of the JVM after the 120-second abandonment; a caller's own 15-second bound around that read, which used to outlast 300 seconds, returns the refusal in 336 ms). Two residuals are named rather than claimed away: a **regular** file on a file system that stops answering, and a path swapped between the description and the open, can each still hold one blocking-pool thread until that open returns — the caller is still freed at the bound, the descriptor is still never leaked, and a caller's own bound cannot be shorter than such an open because acquisition is uncancelable by design, which is the exchange that guarantees the handle comes back. Like `MaxBytes` (row 16) the bound is private to `Resources` and met only as that failure, and the two are chosen the same way: the largest text in this repository is the day-count parity baseline at roughly 12.7 MiB, which reads in well under a second, so a read that is progressing has two orders of magnitude of headroom. A legitimate read slower than this would be refused; none in this tree comes near it | Row 16 bounds how much a source may deliver; this bounds how long it may take not to deliver it **for the caller**, which is one of the two ways a source can cost this process without limit (CWE-400). The other is the thread it sits on, and a time bound cannot answer that where the call cannot be interrupted, which is why the kind of the source is settled before the open instead |
...
| 52 | A run of amounts holds no NaN | `CurrencyAmountArray` and `MultiCurrencyAmountArray` accept any `double` in their values: neither Java source examines an element, so a run could hold a value that is not a number while `CurrencyAmount` itself rejected one (`CurrencyAmount.java:148`) | An element of a run is an amount, and an amount is a number, so the examination sits at the one construction point of each type. The members that are **total in signature** therefore **throw** `IllegalArgumentException` through `ArgCheck`, naming the offending index: `CurrencyAmountArray.of(currency, DoubleArray)`, which AAP §0.4.1 makes total, and the `multipliedBy` and `mapAmounts` of both types, which were total in the types being ported (row 2). Every route that has a failure channel reports it instead, and it is established **once** per value built, by whichever route takes the numbers in, rather than restated in the constructor as the cheaper invariants of this package are: this is the one invariant that costs a pass over the whole run, and a constructor runs for every value built (row 42 and (c)-52 record what that leaves unexamined). The two infinities are values an amount holds and are admitted, and the empty run satisfies the invariant vacuously. See (c)-52 below | A run holding a value that is no amount is a value whose every later reader fails on it, at a distance from where it was built (CWE-20). Establishing the element invariant at the one construction point is what makes the type impossible to hold in that state, and a NaN produced from numbers is the numeric-domain edge AAP §0.3.3 classes as a fail-fast `ArgCheck` throw |
| 53 | `ReferenceDataId` value type | The interface declares exactly one abstract member, `Class<T> getReferenceDataType()` (`ReferenceDataId.java:22`), and the store applies it **at insertion**: `ImmutableReferenceData.validateEntry` tests `id.getReferenceDataType().isInstance(value)` and raises `ClassCastException` for a mismatch (`ImmutableReferenceData.java:96-106`), after which retrieval casts unchecked, commented "no type check against id.getReferenceDataType() as checked in factory" (`:120-122`). Measured: `HolidayCalendarIds.GBLO.getReferenceDataType()` is `com.opengamma.strata.basics.date.HolidayCalendar` | The trait declares `valueType: ReferenceDataType[T]`, and it is the **one** member an implementation must supply — `resolve` and `toReader` have defaults expressed in terms of `ReferenceData` alone — so the obligation is Java's, one abstract member for one, re-expressed as a value that recognises a value **by pattern** instead of as a class token. The check moves from insertion to lookup: `ImmutableReferenceData.findValue` narrows what it found with the witness of the identifier that asked, so **no cast is performed anywhere on the path** and nothing reflects. Measured: `HolidayCalendarIds.GBLO.valueType` names `HolidayCalendar`. **The divergence is from the AAP, not from Java**: §0.4.1 sketches the trait with two members and §0.3.3 describes `findValue` as performing "the one localized cast in the module", where the delivered trait requires the witness and performs no cast. An application implementing the trait supplies it; one written to the two-member sketch does not compile. See (c)-53 | A cast at lookup cannot be made sound, and Java required this member for the same reason: the store is keyed by an identifier whose type argument is erased, the trait is open, and a family parameterized in its value type has instantiations that compare **equal** — `GenericId[String]("x")` and `GenericId[Int]("x")` are one case class over one field — so a cast would answer the second with the value the first filed. The witness is what makes the check the asking identifier's own, and being a pattern match rather than a `Class` token it keeps the store's type test out of reflection altogether, which is the direction Rule 6 and design decision D-5 point in |
| 54 | Zero-day business-day addition through the three-argument factory | `DaysAdjustment.ofBusinessDays(0, GBLO, MF/USNY)` keeps the calendar it was handed: `getCalendar()` is `GBLO` and the text is `0 business days using calendar GBLO then apply ModifiedFollowing using calendar USNY`, while the value's own `normalized()` rewrites that to `0 calendar days then apply ModifiedFollowing using calendar USNY` (all measured) | The factory stores what Java's `normalized()` produces: `calendar` is `NoHolidays` and the text is `0 calendar days then apply ModifiedFollowing using calendar USNY` (measured). Every **computed** value agrees — `adjust(2020-01-01)` is `2020-01-02` and `resultCalendar` is `USNY` on both sides, and `normalized` is identical — so what differs is the unnormalised field, the text form, equality against a value built the Java way, and the JSON, which carries `"calendar":"NoHolidays"`. The validated sibling refuses the pairing outright rather than rewriting it (row 36). See (c)-36 | An addition of zero business days names no day, so the calendar that would have added them is not part of the value. Keeping it leaves a field no computation reads, and two values that compute identically comparing unequal |
| 55 | A composite naming only the no-holidays calendar | `HolidayCalendarId.of("NoHolidays+NoHolidays").getName()` is the **empty string**, and resolving that identifier throws `ReferenceDataNotFoundException: Reference data not found for identifier '' of type 'HolidayCalendarId'` (both measured). Java's `~` branch, given the same request, already answers `NoHolidays` | `of("NoHolidays+NoHolidays")` **is** `HolidayCalendarIds.NO_HOLIDAYS`: its name is `NoHolidays`, it reports itself as not composite, it resolves to the no-holidays calendar against `ReferenceData.standard`, and `of` reads its own name back into it (all measured). Every neighbouring normalisation is Java's, including `NoHolidays+GBLO` → `GBLO`, `"+"` → `""` and `"GBLO+"` → `"+GBLO"` | Combining the no-holidays calendar with itself is the no-holidays calendar, which is the value Java's own `~` branch answers with for the same input, so the port applies one reading to both separators. An identifier with no name is one no store can hold, no text can name and no round trip can recover |
| 56 | Composite identifier construction | `HolidayCalendarId.of` consults a static `ConcurrentHashMap` before it parses anything (`HolidayCalendarId.java:56,87-88`) and files each identifier under both the normalised name and the caller's spelling (`:118,145-146`), so a repeated name is a map hit rather than a parse. Measured: two calls with the same name, and a call with the parts reordered, return the **same instance**, and so does a simple name. The map never evicts, and its keys are whatever text callers passed | Every call parses, normalises, deduplicates and sorts. Equal names give **equal** values and never identical ones, and no global state is kept (measured). A repeat therefore costs the whole of that work — one pass over the name, plus the parts times their logarithm, recursively, since each part goes through `of` itself — where Java's cost a lookup. Measured ad hoc on this build: a simple name ~13 ns, a two-part composite ~200–600 ns, eight parts ~0.7 µs, and the 128 parts `HolidayCalendar.MaxCompositeDepth` admits ~13–26 µs, against ~2 ns to read the name of a value already held. **Hold a composite identifier in a `val`; do not build one inside a loop.** See (c)-56 | The cache is hidden global state of exactly the kind this port removes, and its keys are text a caller supplies, so a document naming a fresh combination on every line grows a static map that is never emptied for the life of the JVM (CWE-770). The work the cache hid is bounded instead (row 48), and a caller that holds the value pays it once. AAP §0.8.4 states no numeric performance requirement and no benchmark is committed with the port, so the figures above are reproducible measurements rather than an asserted budget |
| 57 | Fixing date offset of a mixed-case rate name | `[iborFixingDateOffset]` declares `0` for all three Danish CIBOR rows (`FloatingRateNameData.ini:233-236`), but the loader registers every name twice and writes the offset back under the upper-case key alone: `parseSection` does `put(key, name)` then `putIfAbsent(key.toUpperCase(ENGLISH), name)`, and `parseFixingDateOffset` then `put`s the updated value under `key.toUpperCase(ENGLISH)` (`FloatingRateNameIniLookup.java:125-126,137`). For the row that is already upper case the two keys coincide and the offset reaches the canonical value; for the two mixed-case rows it does not, so **two values share one name**. Measured: `of("DKK-CIBOR-DKNA13-Bloomberg")` answers `OptionalInt.empty` and `-2 business days using calendar DKCO`, `of("DKK-CIBOR-DKNA13-BLOOMBERG")` answers `OptionalInt[0]` and `0 calendar days then apply Preceding using calendar DKCO`, and **both report their name as** `DKK-CIBOR-DKNA13-Bloomberg`. Identically for `DKK-CIBOR-Reference Banks` | One value per name, as a closed family has (row 5), and it is the one the section declares: `DKK-CIBOR-DKNA13-Bloomberg` and `DKK-CIBOR-Reference Banks` each answer `Some(0)` and `0 calendar days then apply Preceding using calendar DKCO`, for every spelling (measured). `DKK-CIBOR-DKNA13` and `DKK-CIBOR2-DKNA13` are Java's exactly, and so is every other row's offset. **A caller of those two names reads a fixing date offset two business days later than Java's** — the one behavioural difference on this surface, and the reason this row exists. The published table is the INI's three rows unchanged, and `manifest/reference-data-manifest.json` carries them as captured. See (c)-57 | Java's state here — one name, two values, told apart by the case the caller spelled it in — is not expressible in a family with one value per name, which is what Rule 4 and the single registration rule of (c)-5 give every family. Of Java's two the port keeps the one the section declares, because the section declares `0` for all three rows and an offset that reaches only an upper-cased spelling of a name is an artefact of how the file was read, not something the file says |
| 58 | Decimal amounts in the API | Every decimal-bearing member takes and returns `java.math.BigDecimal`: `Money.of(Currency, BigDecimal)`, `getAmount`, `mapAmount`, `convertedTo(Currency, BigDecimal)`, the same four on `BigMoney`, `Currency.roundMinorUnits`, `Rounding.round`, and `Decimal.of(BigDecimal)`/`toBigDecimal`/`mapAsBigDecimal` with `FixedScaleDecimal.toBigDecimal`. Measured: `Money.of(GBP, new BigDecimal("12.345")).getAmount()` is a `java.math.BigDecimal` reading `12.35` | The same type on the same members, deliberately, so those members are exact Java parity — measured: `Currency.GBP.roundMinorUnits(new java.math.BigDecimal("12.345"))` answers a `java.math.BigDecimal` reading `12.35`. The consequence for a Scala caller is that `Money.of(Currency.GBP, BigDecimal("12.345"))` with `scala.math.BigDecimal` does **not** compile (measured: `cannot be applied to (Currency, scala.math.BigDecimal)`) and a `mapAmount` function receives the Java type, whose arithmetic is `add`/`multiply` rather than `+`/`*`. The Scala routes are `of(currency, amount: Decimal)` — the port's own decimal, which is what every other member takes — and `of(currency, amount: Double)`, with `scalaValue.bigDecimal` and `scala.math.BigDecimal(javaValue)` converting at the boundary. This is the one place row 4's "Scala-native throughout" is qualified, and section (a) rows 11, 18 and 19 name the `Decimal` members it applies to | `scala.math.BigDecimal` is a wrapper over this very type that also carries a `MathContext` and rounds every operation through it, where the Java type's arithmetic is exact and rounds only where a caller asks: taking the wrapper would either discard the context a caller set, silently changing the rounding they asked for, or make these members disagree with `Decimal`, which is the decimal the rest of the port speaks. Design decision D-3 removes the Java *façade* — no signature is shaped so that Java can call it — not the JDK value types the JDK ships and Scala has no equivalent of, `java.time` being the same choice made in the same way |
| 59 | How a fold of FX rates is accumulated | `FxMatrixBuilder` keeps a `LinkedHashMap` of currency positions and a `double[][]` of rates and mutates both in place, one `addRate` at a time, then hands them to `FxMatrix` at `build()` (`FxMatrixBuilder.java`) | The fold runs over a private, immutable `FxMatrix.Accumulator` — the currency vector, a `HashMap[Currency, Int]` of positions and an `Array[Array[Double]]` whose capacity doubles — which is sealed into the matrix once, at the end. **The divergence is from the AAP, not from Java**: §0.6.6 lists among the `var`-free replacements "`FxMatrixBuilder` mutable maps → fold into `FxMatrix.withRate`", and a fold over `withRate` seals and rebuilds the whole matrix per rate, which is cubic in the number of currencies. Nothing of the accumulator is observable: it is `private`, no `var` is used, the rates it is built from cannot be changed by placing into it, and the matrix it seals reads the leading square of the rows. `withRate` called singly pays the accumulator's set-up — a fraction of a microsecond at two currencies — and is the same value it always was | The plan's intent is what the accumulator keeps: no `var`, an immutable result, one pass. What folding `withRate` adds is the cost of sealing a matrix that is about to be superseded, so a definition naming many rates paid for every prefix of itself. Results are unchanged — bit-identical across the 31 committed FX fixtures, and inside 1e-9 of the Java original — so this row is about the route and not the answer |
...
**What the three-argument factory does with the input the validated one refuses.** It is total in
signature — AAP §0.4.1 gives it no failure channel — so it cannot refuse anything, and it does not
carry the contradiction either: it **rewrites** the request into the value the validated factory
would have accepted, substituting the no-holidays calendar for the one that would have added zero
business days and keeping the trailing adjustment as given. The result is precisely what Java's own
`normalized()` answers for the same request, which is why every computed value agrees while the
value itself differs. Measured on both sides for
`ofBusinessDays(0, GBLO, ModifiedFollowing/USNY)`:

| | Java | Port |
|---|---|---|
| `days` | `0` | `0` |
| `calendar` | `GBLO` | `NoHolidays` |
| `adjustment` | `ModifiedFollowing using calendar USNY` | `ModifiedFollowing using calendar USNY` |
| text form | `0 business days using calendar GBLO then apply ModifiedFollowing using calendar USNY` | `0 calendar days then apply ModifiedFollowing using calendar USNY` |
| `normalized` | `0 calendar days then apply ModifiedFollowing using calendar USNY` | the same, and the value is already it |
| `resultCalendar` | `USNY` | `USNY` |
| `adjust(2020-01-01, standard)` | `2020-01-02` | `Right(2020-01-02)` |
| JSON | — (Joda form, row 30) | `{"days":0,"calendar":"NoHolidays","adjustment":{"convention":"ModifiedFollowing","calendar":"USNY"}}` |

So four things a caller can observe differ — the `calendar` field, the text form, equality against a
value built the Java way, and the JSON — and nothing computed from the value does. A non-zero count
through the same factory keeps the calendar it is given and renders exactly as Java's does, and the
two-argument zero-day form matches Java on both sides. Row 54 is this divergence's entry in the
register, and `DaysAdjustmentSpec` asserts the rewritten value, the refusal of its validated
sibling, and that the dates the two readings produce are the same.
...
- **The published extension points.** `IndexObservation`, `FloatingRate`, `ReferenceData`, `ReferenceDataId`,
  ...
  sensitivity control for every sealing assertion beside it. One of them asks for something of an
  implementor rather than nothing: `ReferenceDataId[T]` requires a `valueType` witness for the type
  of data the identifier names, which is the one member of that trait without a default, and row 53
  with (c)-53 states what it is and why it exists. The other ten are implemented by declaring the
  members the signature already names.
...
| `CurrencyAmount.parse` | `MaxAmountTextLength` = 256 | The figure `Money`, `BigMoney` and `Decimal` already apply to the identical numeral, so one bound now governs all four money-bearing grammars. The longest exact decimal spelling of a finite double is that of the smallest subnormal, 767 significant digits, so unlike the three restatements below **this figure refuses text a shorter spelling cannot always replace**: an amount of 257 to 1 024 characters that named a double exactly is refused for its size, through the wording an unreadable amount already used. What that leaves out is a spelling nothing sends — the suite's own longest legal amount is 256 — and what it buys is that the admitted in-ceiling work is bounded by the same figure everywhere |
| `FxRate.parse` | `MaxRateTextLength` = 256 | The same reasoning, and the same figure. Text past it is reported with the wording of a rate that could not be read rather than one of its own, so the ten-thousand-digit rate of the test suite — refused here for its size where it used to be refused for being zero — still reads as a rate that names no legal value. The **grammar** is read differently from Java's and from this port's first form, and identically in what it accepts: the regular expression applies to the pair prefix alone, which the separator test already bounds at seven characters, and the rate part is read by a `@tailrec` scan over the characters a rate may hold that allocates nothing and stops at the first character that is not one of them. Case folding is therefore applied to the pair and not to the rate, which needs none — no digit, sign or point has an upper case of its own. A 31-case grammar test written before the change and re-run after it pins the accepted set, including an empty rate part, a second space, letters in the rate, a leading `+` and the ligature `ﬁM/USD 1.25`, whose folding expands to `FIM/USD 1.25` |
...
The two figures above are this port's own, and the one on the amount and rate text is the only
ceiling in this row that narrows what a shorter text could also have named. Three further figures in
this area are **Java's own**, restated rather than added, and are recorded here so that a reader
meeting them does not take them for new refusals:
...
- `Money` and `BigMoney` bound the amount text at 256 characters. That is the bound Java's `Decimal`
  ...
  It is also where the figure on the amount and rate text above comes from: the four grammars read
  the same numeral, so they state one bound rather than four.
...
- The invariant is **not** restated on the implementation class, and this is the one place in either
  module where a validated type's class body does not restate what its factory established. The
  implementation carries a public constructor in the class file whatever the source asked for
  (row 42), so a restatement there is what would also hold of an instance forged by a class file
  naming that constructor directly — but this invariant costs a pass over the whole run, and a
  constructor runs for every value built, so restating it would examine every element of every
  array a second time and double the cost of `of` and of each element-wise operation on a run of a
  hundred thousand scenarios. The class body carries that reasoning at the point the restatement
  would go, `JvmClosure.requireSoleImplementation` is still called there so a foreign *subtype* is
  still refused, and the acceptance gate records the exemption by name in the one place it keeps
  them. What is therefore unexamined is a forged instance — a route design decision D-3 does not
  support — and a value that is not a number in one is refused by `CurrencyAmount` as soon as any
  element of it is read as an amount.
- `MultiCurrencyAmountArray` does the same with the same split between raising and reporting, and
  **does** restate its two structural invariants in its class body — that its size is not negative,
  and that it holds exactly one value per index of the run for each of its currencies — because
  those cost a constant and a step per currency, and they are what stop a forged run being read
  past the end of one of its arrays. Only its per-element walk is established once, as above. Its
  scan is per currency, in the order the run holds them, and it is an iterator rather than a
  collection so that the raise reads only the first offending entry while the accumulating route
  reads all of them.
...
### (c)-53 — The value type an identifier carries, and what an implementation supplies

AAP §0.4.1 sketches the identifier trait as two members, `resolve(refData)` and `toReader`, and
§0.3.3 describes the store's `findValue` as performing "the one localized cast in the module,
justified because entries can only be inserted with matching id and value types". The delivered
trait has a third member, `valueType: ReferenceDataType[T]`, it is the only one an implementation
must write, and no cast is performed at lookup or anywhere else. That is the divergence, and it is a
divergence from the plan rather than from Java: Java's `ReferenceDataId<T>` also declares exactly one
abstract member for exactly this purpose, `Class<T> getReferenceDataType()`.

**Why the cast the AAP names cannot be made sound.** The justification it gives holds for insertion
and only for insertion. A value can enter a store only as a `ReferenceData.Entry[T]` or through a
factory whose key is an identifier of the value's type, so a value is never *filed* under an
identifier of another type — that much is settled where the store is built, and it remains settled.
What it does not settle is what a lookup *finds*. The store is a map keyed by an identifier whose
type argument is erased, a lookup compares keys with ordinary `equals`, and the trait is open, so a
host may declare a family parameterized in the type of data it names — which is what a host writes
as soon as it holds data of more than one type. Two instantiations of such a family are one key:
`GenericId[String]("x")` and `GenericId[Int]("x")` are one case class over one field with the type
argument gone, and they compare equal and hash equal. A cast would answer the second with the
`String` the first filed, at the type the caller asked for, and fail somewhere else entirely.

**What the witness is.** A `ReferenceDataType[T]` is a name and a partial function that recognises a
value of `T`; `ReferenceDataType.of("java.lang.Number") { case value: Number => value }` is a whole
one. `ImmutableReferenceData.findValue` finds the entry by equality and then narrows the value with
the witness of the identifier that asked, so a store holding a value the asking identifier does not
recognise reports an **absence** rather than handing it back mistyped — and `combinedWith` then
consults the next source, which is how a store that does hold the requested type still answers. The
narrowing is a pattern match, so nothing reflects and Rule 6 is satisfied, where Java's `Class`
token and `isInstance` were a reflective check made once per entry at insertion.

**What an application must supply.** One member. An identifier of a fixed type of data names the
witness for that type, as `HolidayCalendarId` names `ReferenceDataType.holidayCalendar`; a family
parameterized in its value type demands one per instantiation, typically as an implicit constructor
parameter, which is what makes its instantiations distinguishable at retrieval although they compare
equal. `resolve` and `toReader` keep their defaults, so an identifier that is no more than an
identity is a `final case class` with one `override def valueType`. An implementation written to the
two-member sketch does not compile, and the diagnostic says so — `class … needs to be abstract`.
`ReferenceDataSpec` asserts both halves, the first over the emitted trait: `valueType` is its only
abstract member while `resolve` and `toReader` are concrete, so a class omitting it cannot be
concrete; and supplying it is the whole of implementing the trait, which compiles and retrieves. The
rejection is asserted that way rather than with `assertDoesNotCompile`, because that macro
type-checks its snippet and stops, while `needs to be abstract` comes from a later compiler phase —
which is why an ordinary compile, and not the suite, is what surfaced the requirement.

### (c)-56 — Composite identifiers are constructed, not cached

Java's `HolidayCalendarId.of` is a cache lookup first and a parser second. A static
`ConcurrentHashMap` is consulted before the name is examined at all, and `create` files the finished
identifier under the normalised name **and** the caller's spelling, so `of("USNY+GBLO")` and
`of("GBLO+USNY")` are afterwards the same instance, as are two calls with either spelling and every
simple name a program has ever passed. The map has no eviction and no bound.

This port keeps no such map. `of` splits the name, resolves each part through `of` itself,
deduplicates, sorts and rejoins, every time it is called, and answers with a value that is **equal**
to the one the previous call answered with — equal name, equal components, equal hash, equal
resolution — but not the same instance. Nothing about a caller's use of an identifier depends on
that identity: the type is compared by `equals`/`Eq` and never by reference, and a store is a map
keyed by it.

What does depend on it is cost, and the difference is a repeat: the first construction of a name
costs the same on both sides, and every later one costs a lookup in Java and the whole of the work
here. Measured ad hoc on this build — reproducible by the procedure in
[`tools/parity-capture/README.md`](tools/parity-capture/README.md), not committed as a benchmark, and
noisy on a shared host — a simple name costs about 13 ns, a two-part composite 200–600 ns, eight
parts around 0.7 µs and the 128 parts `HolidayCalendar.MaxCompositeDepth` admits 13–26 µs, against
about 2 ns to read the name from a value already in hand.

**The caller contract, therefore:** hold a composite identifier in a `val` — every composite this
library itself names is one — and do not construct one inside a loop or per row of a document. A
value held is free to use, and `HolidayCalendarIdSpec` asserts what that relies on: that two
constructions of one name agree in name, parts, equality, hash and resolution, so hoisting the
construction out of a loop cannot change an answer.

The trade is deliberate. A static map keyed by text a caller supplies is hidden global state of the
kind this port removes throughout, and it grows: a feed naming a fresh combination of centres on
every line adds an entry per line that is never emptied while the process runs (CWE-770). What the
cache also hid was unbounded work, and that is bounded here instead — a name past
`MaxCompositeNameLength` or `MaxCompositeParts` is kept whole rather than decomposed (row 48). AAP
§0.8.4 states no numeric performance requirement for this library, which is why this row records a
cost rather than a regression against a budget.

### (c)-57 — The fixing date offset of the two mixed-case Danish CIBOR names

`[iborFixingDateOffset]` declares three rows, and all three declare zero:
...
Java does not apply all three. Its loader registers every rate name twice — the spelling the file
uses, unconditionally, and the English upper-case of that spelling where the key is still free — and
then writes each offset back under the **upper-case key only**. `DKK-CIBOR-DKNA13` is already upper
case, so its two keys coincide and the offset reaches the value the file names. The other two are
not, so the offset reaches a value reachable only by shouting the name, and the file's own spelling
keeps a value with no offset. Measured against the `2.12.74-SNAPSHOT` jars:

| Java lookup | `getFixingDateOffsetDays()` | `toIborIndexFixingOffset()` | `getName()` |
|---|---|---|---|
| `of("DKK-CIBOR-DKNA13")` | `OptionalInt[0]` | `0 calendar days then apply Preceding using calendar DKCO` | `DKK-CIBOR-DKNA13` |
| `of("DKK-CIBOR-DKNA13-Bloomberg")` | `OptionalInt.empty` | `-2 business days using calendar DKCO` | `DKK-CIBOR-DKNA13-Bloomberg` |
| `of("DKK-CIBOR-DKNA13-BLOOMBERG")` | `OptionalInt[0]` | `0 calendar days then apply Preceding using calendar DKCO` | `DKK-CIBOR-DKNA13-Bloomberg` |
| `of("DKK-CIBOR-Reference Banks")` | `OptionalInt.empty` | `-2 business days using calendar DKCO` | `DKK-CIBOR-Reference Banks` |
| `of("DKK-CIBOR-REFERENCE BANKS")` | `OptionalInt[0]` | `0 calendar days then apply Preceding using calendar DKCO` | `DKK-CIBOR-Reference Banks` |

The third and fifth rows are not aliases answering with the second and fourth. They are **different
values that report the same name**, and which one a Java caller gets depends on the case it spelled.
The offset-bearing value of each mixed-case row is therefore unreachable by the name it reports.

The difference reaches no other rate name, which is measurable rather than assumed: of the 454 keys
Java's registry answers and the 351 distinct values behind them, exactly three carry a fixing date
offset at all, and they report the three names above — so every other name defers to the offset of
the index it names, identically on both sides.

A closed family cannot hold that state, and this port does not try to: one name has one value
(row 5 and (c)-5, where the single registration rule is stated). Of Java's two values it keeps the
one the section declares, so every spelling of either name answers `Some(0)` and
`0 calendar days then apply Preceding using calendar DKCO`. `DKK-CIBOR-DKNA13` is unchanged, and so
is `DKK-CIBOR2-DKNA13`, which declares no offset and keeps its index's own `-2 business days using
calendar DKCO`.

**The consequence is a shifted date and it is worth stating plainly.** `toIborIndexFixingOffset` is
the offset a caller applies to reach the fixing date of a DKK CIBOR rate. For those two published
names this port answers with the day itself rolled back to the preceding business day, where Java
answered with two business days earlier. Every other rate name on this surface, and every other
member of these two, is identical to Java. The published table is the file's three rows verbatim,
`manifest/reference-data-manifest.json` carries them as captured from Java, and
`FloatingRateNamesSpec` asserts the port's answers with this row named beside them, so the
assertions read as the deliberate departure they are rather than as parity.
...
- **Sealed families use the wrapper object** keyed by the constructor name — `{"HalfUp":{…}}`,
  ...
  `circe-parser` and nothing else, and only the first two are on either module's Compile classpath —
  no main source reads JSON, so `circe-parser` is declared `% Test` and the parsing artefacts are on
  the two Test classpaths alone (section (g), row 17). `Failure` derives that wrapper; `Rounding`
  builds it by hand with `Encoder.instance`, and `DayCount` and `HolidayCalendar` are hand-written
  throughout, because each of those three mixes forms that no single derivation produces.
...
| `Named`, `NamedEnum`, `TypedStringCompanion`, `ArgCheck`, `Validate`, `Collections`, `DoubleArrayMath`, `Resources`, `Codecs`, `ReferenceDataType` | Typeclasses, helpers and effects, not data. `ReferenceDataType[T]` is the value witness a `ReferenceDataId[T]` carries — a name and a partial function that recognises a value (see (c)-53) — so it holds no data of its own |
...
| `Gate 1a - test-report recovery on a domain-valued failure` | A run in which a table-driven check fails over a domain value still ends, still exits non-zero, names the suite that failed, renders the falsifying row (`dayCount = Act/360`) to the console, and leaves exactly one complete `TEST-*.xml` for it (`tests=3 failures=1 errors=0`), which the row then removes |
...
removed or re-versioned any of them. Two of those eleven declarations have since been changed, and
both changes only subtract from a resolved classpath: row 9's exclusion, which drops a duplicate
transitive, and row 17's `% Test` on `circe-parser`, which narrows one coordinate's configuration and
takes it and its two transitives off both Compile classpaths while leaving both Test classpaths as
they were. Nothing else about a declaration has ever changed, and neither change adds anything:
...
# eleven lines each; the two that differ are discipline-scalatest, parenthesised to carry its
# exclusion, and circe-parser, which gained `% Test`
```
...
tooling-security and comment reviews of the finished tree. Rows 13 to 16 are the CI-definition review
of the finished `scala_build21` job, and two of them correct rows above rather than only adding to
them: row 13 withdraws the comment change row 11 made to a release-surface command, and row 16
corrects what row 8 said about the cached paths. Rows 17 and 18 are the dependency and supply-chain
security review of the delivered tree — the review that found the surface clean and raised only
hardening and process matters — and row 17 is the second of the two declaration changes the
invariant above names. Rows 19 and 20 are the CI supply-chain review of the CI definition — how a
secret reaches `gpg`, and how the base image the Scala job runs on is pinned — and row 19 corrects
row 13 in its turn: its fix changes a pre-existing line, so row 13's claim that the port's change to
that file is purely additive no longer holds, and that row carries the measured one-line difference
instead. Row 21 is the final QA acceptance review of the running build, whose one finding is about
the test report itself: a failure carrying a domain value hung the test task and stranded its
evidence, which is why the build no longer reports through ScalaTest's slave-to-master socket and why
the runner has a row that provokes that failure on purpose. **Impact** is measured rather than assumed
...
| 2 | `config/F02` — LOW: the custom log appender opened a `PrintStream` per logger with no deterministic close | sbt Build and Repository Configuration | `build.sbt` | Removed with the same layer; test reporting is a single JUnit XML listener, which owns its files (ScalaTest's own `-u` reporter until row 21 replaced it with sbt's listener) | none | `86d8c8a32` |
...
| 4 | `config/F04` — LOW: `IO.createDirectory` mutated the filesystem during setting evaluation | sbt Build and Repository Configuration | `build.sbt` | Removed; the report directory is created when a test task starts rather than while a setting is evaluated (by the `-u` reporter itself until row 21, by `prepareTestReports` since) | none | `86d8c8a32` |
...
| 8 | `SECTOOL-F16` — MEDIUM, blocking: `save_cache` stored the whole `~/.sbt`, the standard home of `credentials.sbt` and `.credentials`, and no scanner examines cache contents | Tooling, Artifact and Secret Security | `.circleci/config.yml` | The cached path is the Coursier download cache alone, `~/.cache/coursier`, as row 7 states and as the job's own comment above `save_cache` states: nothing under `~/.sbt` is cached at all, so the standard home of `credentials.sbt` and `.credentials` is never stored, and the validation step of row 7 refuses a credential file that arrives from anywhere else. Until row 16 corrected it against the delivered step, this row read "the two download caches only — `~/.cache/coursier` and `~/.sbt/boot`", which was never what the job cached | none to any classpath | this revision |
...
| 11 | `CMTBLD-F01`–`F04`, `F87`, `F88` — six LOW, two blocking: migration narration in the `build.sbt` banner and the CI job banner, a paraphrase of the visible compiler options, a duplicated report-path explanation, a comment duplicating README material, and a time-sensitive GPG version claim | COMMENTS — Build, Tooling, CI and Documentation | `build.sbt`, `.circleci/config.yml` | Comment text only: the narration is gone and the surviving comments state the root-project, fork-lifecycle, cache and gate-ownership invariants. The GPG item is the one part of this row that no longer stands: the comment it rewrote sits above the pre-existing `setup_gpg` reusable command, which belongs to the release surface AAP §0.2.2 places out of scope, so row 13 reverts it to its pre-port text and that item's remediation is withdrawn — the time-sensitive version claim stays as upstream wrote it, in a block this port does not own | none — no setting, coordinate or step semantics changed | this revision |
...
| 13 | `CI-SETUPGPG-COMMENT-EDIT` — LOW: the port's only non-additive change to `.circleci/config.yml` was a comment above the pre-existing `setup_gpg` command — the single removed line in the whole diff against the pre-port file `7e11df417` — so "the Maven jobs are not modified" held for every job, executor and command definition but not as byte-equality of the file outside the new job | Scala Build CI Infrastructure — CI definition (4 findings, all LOW, 2 blocking) | `.circleci/config.yml`, `SCALA_MIGRATION.md` | The comment is reverted to its pre-port text, and row 11's GPG item is withdrawn with it (see that row): a comment on the release surface AAP §0.2.2 excludes is not this port's to improve. That much stands. What this row used to add does not, and is corrected here rather than left standing: it claimed the port's change to this file was "now purely additive outside `scala_build21`", evidenced by `git show 7e11df417:.circleci/config.yml \| diff -u - .circleci/config.yml` reporting no removed line and three remaining hunks. Row 19 replaces a pre-existing line, so the measured diff against `7e11df417` now carries **exactly one removed line** — the `setup_gpg` passphrase-transport line — beside 246 added lines in four hunks: the new `jdk21_pinned` executor (row 20), that one replacement, the new job, and its `build`-workflow entry. That line is the port's to change: the AAP protects the Maven jobs and their environments — untouched and byte-identical, as row 20 records — not byte-equality of a file the §0.10.1 repository-boundary gate deliberately omits, and the line is a secret-handling defect rather than a comment preference, one the old form silently corrupts four of seven passphrase classes with (row 19) | none — a comment on a command no Scala job uses | this revision |
| 14 | `CI-ARTIFACT-PATHS-NOT-AAP-NAMED` — LOW, blocking: AAP §0.4.1 and §0.10.1 name four artifact paths — `target/gate-report.md`, `target/parity-report`, `target/test-reports`, `target/audit` — and the job stores none of them under its own name; it has one `store_artifacts` (`target/publish`, destination `gate-evidence`) and one `store_test_results` (`target/publish/test-reports`), so the documented contract and the delivery disagreed | Scala Build CI Infrastructure — CI definition | `.circleci/config.yml`, `SCALA_MIGRATION.md` | Reconciled towards the delivery, not the path list, because uploading those four paths directly is exactly what the credential scan exists to prevent — they hold the unsanitized originals, including the runner's hostname, account, home, checkout and toolchain paths, and the upload steps run `when: always`, which is when a scan has just objected. What is published is stated in both places a reader looks — the comment above the upload steps in the job, and here: everything the four paths hold **is** published, as sanitized copies inside the one `target/publish` tree, arriving as `gate-evidence/gate-report.md`, `gate-evidence/parity-report/`, `gate-evidence/test-reports/` and `gate-evidence/audit/`, beside `MANIFEST.txt` (a sha256 per published copy) and `PUBLICATION-STATUS.txt` (the verdict and what was withheld); `store_test_results` reads `target/publish/test-reports`. Nothing a gate run produces goes unpublished, so an operator following the specification's list finds each of its four entries one level down, under `gate-evidence/` | none — an artifact destination, not a dependency | this revision |
| 15 | `CI-STEP10-INSECURE-TMPFILE` — LOW, blocking (security): the publication step wrote the uploader check's output to the fixed path `/tmp/verify-publication.txt` and read it back from there, so a symbolic link pre-created at that path was followed and the file it pointed at was overwritten while the step still exited 0 (CWE-377), two concurrent runs on one host shared the single path — which could put the other run's lines into `PUBLICATION-STATUS.txt` — and the file outlived the step | Scala Build CI Infrastructure — CI definition | `.circleci/config.yml` | The step captures the check's output through a pipe into a shell variable and quotes it from there, so there is no path to pre-create, nothing is shared between two runs, nothing is left behind, and the step again writes nowhere outside `target/`. Everything else about it is unchanged and still verified: a non-zero verdict deletes the tree and replaces it with the `PUBLICATION: NOT APPROVED` notice carrying the check's first forty lines, `target/publish/test-reports` is recreated so neither upload step can fail on a missing path, and the step itself never fails the job — the gate run's exit status remains the verdict on the gates | none | this revision |
| 16 | `MIGDOC-CACHE-PATHS-MISSTATED` — LOW: row 8 of this ledger claimed the job caches `~/.cache/coursier` and `~/.sbt/boot`, while the delivered `save_cache` caches `~/.cache/coursier` alone — so the ledger contradicted both row 7 and the job's own comment, and one of its statements was false about the delivered job | Scala Build CI Infrastructure — CI definition | `SCALA_MIGRATION.md` | Row 8 now names the single cached path, and says what it used to claim so the correction is visible rather than silent. The job is what it should be and was not changed: sbt's boot jars have no published checksum for the restore-time verification step to check them against, which is the reason the job gives for caching nothing under `~/.sbt` | none | this revision |
| 17 | `DEP-circe-parser-compile-scope` — LOW, not blocking: `circe-parser` was declared in `commonSettings` with no configuration, so it sat on both modules' Compile classpaths although no main source of either module reads JSON, and it was the only Compile route to `circe-jawn` and `jawn-parser` — the one artefact of the forty-one resolved with any advisory history, unaffected at 1.7.0 | Dependency and Supply-Chain Security Surface (2 findings, both LOW, neither blocking) | `build.sbt`, `SCALA_MIGRATION.md` | The declaration gained `% Test`, and the reason is stated at the declaration as well as here. It was measured before it was decided: the two main trees import `circe-core` (`Encoder`, `Decoder`, `Codec`, `Json`, `KeyEncoder`, `KeyDecoder`, `HCursor`, `ACursor`, `CursorOp`, `DecodingFailure`, `syntax.EncoderOps`) and `circe-generic` (`semiauto`) and nothing else — `io.circe.parser` appears in **0** main sources and **43** test sources, and the demo encodes only (`asJson.noSpaces`) — so the coordinate is a test dependency by use, while Compile scope also published it to anything that would depend on these two libraries. This is a **deliberate divergence from AAP §0.5.1**, which tabulates `circe-parser` as "both, Compile" for "round-trips, fixtures and the demo": round-trips and fixtures are test scope and the demo does not parse, so the stated purpose is met at `Test`. Section (e) states the narrowed scope where the codec shapes are described | measured, and the second classpath change in this ledger: `strata-collect/Compile` 15 entries → 12 and `strata-basics/Compile` 16 → 13, each losing exactly `circe-parser_2.13:0.14.16`, `circe-jawn_2.13:0.14.16` and `jawn-parser_2.13:1.7.0` and gaining nothing; both Test classpaths unchanged at 39 and 45 entries with all three still on them; no coordinate added, removed or re-versioned; Gate 2's forbidden-pattern count stays 0 and its `scala-library`, `cats-core` and collect-`classes` positive controls stay present on all four classpaths | this revision |
| 18 | `DEP-no-advisory-monitoring` — LOW, not blocking: nothing continuously watches the port's dependency surface — `.github/dependabot.yml` declares the `maven` ecosystem alone, no scanner step runs in CI, and no review policy was written down anywhere — so a future advisory against a resolved coordinate, a superseded sbt pin or a stale tarball digest would surface nowhere | Dependency and Supply-Chain Security Surface | `SCALA_MIGRATION.md`, `README.md`, `strata-basics/README.md`, `strata-collect/README.md` | Section (h) below, with a pointer from each of the three READMEs: what the surface consists of, the commands that review it, when the review is due, the date and verdict of the last one, and the automation an owner would add. What is deliberately **not** done: an `sbt` ecosystem entry belongs in `.github/dependabot.yml`, which AAP §0.2.2 places out of scope and §0.10.1's repository-boundary gate requires to stay byte-unchanged (`git status --porcelain -- modules examples eclipse pom.xml src .github` empty), so it is recommended in section (h) and not applied; a scanner step belongs to `.circleci/config.yml`, which this row does not touch. The residual risk is reduced to a documented manual cadence rather than closed, and section (h) says so in those words | none — no coordinate, configuration or classpath entry changed | this revision |
| 19 | `CI-gpg-secret-shell-pipeline` — LOW, non-blocking: the pre-existing `setup_gpg` command handed `$SIGNING_PASSWORD` to `gpg` through a shell pipeline, `echo $SIGNING_PASSWORD \| gpg --batch --passphrase-fd 0 …`, and unquoted — so the secret crossed a subshell, and bash word-split and glob-expanded it against the step's own `working_directory`, `~/repo/.circleci`, which holds `codesigning.asc.gpg`, `config.yml` and `maven-settings.xml` | Dependency Safety and Software Supply Chain — CI supply chain (4 findings, all LOW, all non-blocking) | `.circleci/config.yml`, `SCALA_MIGRATION.md` | That one line now hands the passphrase to `gpg` on a file descriptor, quoted, from a process substitution: `gpg --batch --passphrase-fd 3 -o codesigning.asc -d codesigning.asc.gpg \` continued by `3< <(printf '%s' "$SIGNING_PASSWORD")`. A pipe, so nothing is written to disk — which is why `--passphrase-file`, the finding's other suggestion, was rejected: it would put the secret in a file inside the checkout. Nothing else in the command is touched: `gpg --version`, the import, the `shred` and the three `gpg.conf` appends stand as upstream wrote them, and no gpg flag was added, because this is working release-surface code against a gpg version the tree cannot exercise. Measured with gpg 2.4.8 against a replica symmetric AES256 artifact in a replica of that working directory: across seven passphrase classes the old line decrypts three and fails four — two consecutive spaces, a tab, leading and trailing spaces, and a word that globbed to `config.yml` — while the new line decrypts all seven to byte-identical plaintext; sampling `/proc/<pid>/cmdline` throughout a decrypt found the passphrase in no argv over 14,260 samples that saw the `gpg` process itself 521 times, against 756 hits for a `--passphrase <value>` control | none — no declared coordinate changed and no entry joined or left a resolved Compile or Test classpath: this is a release-surface step no Scala job invokes and no build resolution reads | this revision |
| 20 | `CI-floating-base-image-tag` — LOW, non-blocking: `scala_build21` ran on the `jdk21` executor's floating `cimg/openjdk:21.0` tag, with no digest or full-version pin, and the job's own guard `java -version 2>&1 \| grep -q '"21'` asserted the major version alone — so the JDK patch level, its vendor and the OS layer the gates run on could all change with no change to the repository | Dependency Safety and Software Supply Chain — CI supply chain (4 findings, all LOW, all non-blocking) | `.circleci/config.yml`, `SCALA_MIGRATION.md` | Prevention and detection. A new executor `jdk21_pinned` — added beside `jdk21`, not in place of it — pins the image by digest, `cimg/openjdk@sha256:a94c24460ede1270ab32fbad52fd8afaf8a46113667fe6e7e3d4ce92442960e2`, which is what `cimg/openjdk:21.0` and `cimg/openjdk:21.0.12` both resolve to, so the digest is the full-version pin too and needs no tag beside it (an OCI image index, so both architectures are covered); it carries the same `working_directory` and the same `JAVA_TOOL_OPTIONS: -XX:MaxRAM=3572m` this file's own comment calls vital, and no `MAVEN_OPTS`, because the job runs no Maven. `scala_build21` is the only job on it. Its "Checking Java version" step now asserts one declared expectation, `21.0.12`, against two independent sources — the `JAVA_VERSION` the pinned image declares and what the JVM reports — and fails naming which disagreed: executed, a patch-and-vendor drift to 21.0.15 on Zulu that the old grep passes fails here, as do an unset declaration and a JDK 22. The residual is deliberate and recorded rather than fixed: the four executors the Maven jobs use keep their floating tags, since AAP §0.2.1, §0.4.1 and §0.5.2 require those jobs and their environments to be untouched — verified identical here, textually and parsed — so pinning them is upstream's to do | none — no declared coordinate changed and no entry joined or left a resolved Compile or Test classpath; the pin decides which image bytes a CI job runs on, not what the build resolves | this revision |
| 21 | `qa/F01` — HIGH, blocking: a ScalaTest table- or property-check failure whose row held a domain value hung the test task forever and stranded its report. Forked tests reported through ScalaTest's slave-to-master socket, which carries each event as a Java-serialized object; no type in this port takes part in Java serialization, so the write threw inside `ObjectOutputStream`, the sbt side read a corrupt stream and returned to `ServerSocket.accept`, and the unbounded `Thread.join()` that ends `ScalaTestRunner.done()` never returned — no verdict, no failed-test list, and no authoritative `TEST-*.xml` for the suite that failed | Final QA acceptance testing (1 finding, HIGH) | `build.sbt`, `scripts/verify-gates.sh`, `SCALA_MIGRATION.md` | `build.sbt` wraps every framework from `Test / loadedTestFrameworks` in `withoutRemoteReporting`, whose runner answers `remoteArgs()` with an empty array and never asks the delegate for its own — asking is what opens the socket. The fork then runs ScalaTest in ordinary master mode and reports through the `EventHandler` and `Logger`s sbt gave it, which cross the fork boundary as sbt's own `ForkEvent`/`ForkError`: names, statuses and rendered messages, never domain objects. With the socket gone, ScalaTest's `-u` reporter would silence its own sbt-log reporting (ScalaTest logs through sbt only while no reporter of its own is configured), so it is no longer configured and sbt's `JUnitXmlTestsListener` — retargeted onto the one configured directory — is the single authoritative writer: same file name, same `tests`/`failures`/`errors` attributes and the same `<testcase>` `classname` and `name`, verified identical over all 102 suites and 5,939 cases. That is this build's one deliberate divergence from AAP §0.3.1's wording, which names the `-u` reporter; what the wording is for — one directory of `TEST-<suite>.xml` whose `tests` attributes the test-count gate sums — is unchanged. The staging directory and the promotion path went with it, leaving `reportAuditingTestResultLogger` as a pure completeness check that fails the task when a suite that ran has no fresh report covering at least the cases and failures sbt counted. `scripts/verify-gates.sh` gained `Gate 1a`, which provokes exactly that failure under a 300s bound and requires a prompt non-zero exit, a complete report, and the falsifying domain value rendered to the console — the payload the defect destroyed — and Row 1 now reads the run's size from sbt's own result line and fails if any socket-reporter failure appears in the log | none — no declared coordinate changed; `Test / testOptions` lost one ScalaTest argument and no classpath entry moved | this revision |
...
**Re-checking a row.** The commands below establish rows 1 to 6 from history, rows 7 to 12 from the
tree as it stands, and rows 13 to 16 and 19 to 20 from the CI definition itself; row 17 is
established by the coordinates diff above and row 18 by section (h)'s own re-check block:
...
git show 7e11df417:.circleci/config.yml |
  diff -u - .circleci/config.yml | grep -c '^-[^-]'           # row 13: 1 removed line - the setup_gpg passphrase
grep -n 'destination: gate-evidence' .circleci/config.yml     # row 14: the one upload destination
grep -c /tmp .circleci/config.yml                             # row 15: 0 - no fixed temporary path
sed -n '/save_cache/,/coursier/p' .circleci/config.yml        # row 16: the one cached path
git grep -nE 'echo +"?\$[A-Z_]{3,}' -- .circleci/config.yml   # row 19: no hit - no secret through a pipe
grep -n -A3 'jdk21_pinned:' .circleci/config.yml              # row 20: the digest the Scala job is pinned to
grep -n 'executor: jdk21' .circleci/config.yml                # row 20: which job is on which of the two
```

## (h) Dependency review and advisory cadence

Sections (a) to (g) describe what was ported and what it cost. This section describes the one thing
that has to keep happening afterwards, because the port introduced a second build whose dependency
surface nothing in this repository watches.

`.github/dependabot.yml` declares the `maven` ecosystem alone: it opens pull requests against the
POMs under `modules/**` and does not see `build.sbt`, `project/build.properties` or
`project/plugins.sbt`. No scanner runs in CI. The gates of §0.10.1 measure dependency *purity* —
Gate 2 proves no Guava, no Joda and no Java `strata-collect` jar reach either classpath — and say
nothing about whether a resolved version is current or carries an advisory. Those are different
questions, and the second one has no automated answer here.

So this section is the review that is available within the plan's scope. It reduces the risk of the
port's dependency surface aging silently to a documented, repeatable manual cadence. It does not
close it: a manual cadence that nothing schedules depends on somebody performing it.

### The surface under review

- **The eleven declared coordinates** in `build.sbt`'s `commonSettings` and the `strata-basics`-only
  `Test` block, at the versions held in the `lazy val`s above them: cats 2.13.0, cats-effect 3.7.1,
  circe 0.14.16, ScalaTest 3.2.20, ScalaCheck 1.20.0, scalatestplus `scalacheck-1-20` 3.2.20.0,
  cats-effect-testing-scalatest 1.8.0, cats-laws 2.13.0, discipline-scalatest 2.3.0. Nine of the
  eleven are in `commonSettings` and apply to both modules; the two law-checking libraries are
  declared on `strata-basics` alone, because only it hosts `TypeclassLawsSpec`.
- **The forty-one distinct jars resolution produces from them.** Eleven reach a Compile classpath —
  `scala-library`, `cats-core`, `cats-kernel`, `cats-effect`, `cats-effect-kernel`,
  `cats-effect-std`, `cats-mtl`, `circe-core`, `circe-generic`, `circe-numbers`, `shapeless` — and
  the remaining thirty are on the Test classpaths only, which is three more than before row 17
  re-scoped `circe-parser`.
- **sbt itself**: the `sbt.version=1.13.0` pin in `project/build.properties`; the bundled
  `addDependencyTreePlugin` in `project/plugins.sbt`, which resolves no external artefact; and the
  pinned launcher tarball the CI job fetches, whose URL and `sha256` digest are literals in
  `.circleci/config.yml`. A digest pinned to a URL is only as good as the last time somebody
  re-checked that the URL still serves those bytes.
- **The compiler and the runtime**: Scala 2.13.18, and the Temurin JDK 21 the build targets with
  `-release 21`. In CI that runtime is fixed by the base-image digest section (g) row 20 records,
  and the `scala_build21` job asserts the `21.0.12` that image declares, so a review re-checks that
  digest exactly as it re-checks the launcher tarball's.

### When to review

Before a release; whenever `build.sbt`, anything under `project/`, or the CI toolchain-installer
step changes; and otherwise quarterly. Nothing in the repository schedules any of those — that
absence is the residual risk this section records rather than removes.

### What a review runs

```bash
# 1. What is actually on each classpath, and the paths each coordinate came in by.
sbt -batch "strata-collect/Compile/dependencyTree" "strata-collect/Test/dependencyTree" \
           "strata-basics/Compile/dependencyTree"  "strata-basics/Test/dependencyTree"
sbt -batch "export strata-collect/Compile/fullClasspath" "export strata-collect/Test/fullClasspath" \
           "export strata-basics/Compile/fullClasspath"  "export strata-basics/Test/fullClasspath"
sbt -batch "strata-basics/evicted" "strata-collect/evicted"     # conflicts resolution settled silently

# 2. The purity and boundary gates, which must stay green across any version bump.
scripts/verify-gates.sh                                          # Gate 2 / 2a rows in particular
```

Then three judgements the commands above do not make:

1. **Advisories.** Take the coordinate list from the four classpath exports and check each against
   OSV and the GitHub Advisory Database. Any scanner that reads a jar list does this — none is
   committed here and no gate requires one, because adding a scanner step means editing
   `.circleci/config.yml`, which this section is not the owner of.
2. **sbt currency.** Compare the `project/build.properties` pin against the current sbt 1.x release.
   sbt advisories are build-time and launcher-scoped rather than shipped in the artefact, so the pin
   is the whole of the exposure.
3. **Tarball digest.** Re-download the pinned launcher URL and confirm the `sha256` in
   `.circleci/config.yml` still matches. A mismatch is either an upstream re-publish or a
   compromise, and both are worth knowing about.

### What the last review found

Reviewed 2026-09-13 against the tree at that revision: forty-one resolved coordinates across ten
publisher organisations, and **no applicable advisory**.

- `jawn-parser_2.13` was the only coordinate with any advisory history at all. Its known issue
  (CVE-2022-21653 / GHSA-vc89-hccf-rq55, a hash-collision denial of service) is fixed well below the
  resolved 1.7.0 and reaches only code that extends `SimpleFacade` or `MutableFacade` directly, which
  circe's facades do not; this port implements no facade of its own. Since row 17 it is on neither
  Compile classpath.
- sbt 1.13.0 is itself the remediated version for the BSP-token advisory and sits above the affected
  ranges of the other sbt CVEs. Every command in this document and in CI uses `sbt -batch`, which
  starts no build server, so the BSP surface is not reachable regardless.
- `scala-library` 2.13.18 is far above the 2.13.9 that fixed CVE-2022-36944, whose exploit path is
  Java deserialization — which the port refuses in both directions: `NoJavaSerialization` is mixed
  into its value types (168 mix-ins across 64 main sources) and refuses `writeReplace` and
  `readResolve` alike, and nothing on the codec path reflects (Rule 6, section (e)).

Those three are restated here from that review rather than re-measured by this section; the commands
above are what re-measures them.

### The automation an owner would add, and why it is not in this change

- **An `sbt` ecosystem entry in `.github/dependabot.yml`.** This is the obvious first step, with one
  limitation worth stating before anyone relies on it: Dependabot's sbt support covers *version*
  updates, not *security* updates, so it keeps pins fresh but will not raise an advisory.
- **Scala Steward**, which is what the upstream libraries this port depends on use for their own
  dependency updates, for the same version-currency purpose with better Scala-ecosystem coverage.
- **A scanner step in CI** over the jar list the classpath exports produce. This is what actually
  closes the advisory half, and it belongs to `.circleci/config.yml`.

None of the three is applied here. `.github/**` is out of the plan's scope (§0.2.2) and frozen by the
repository-boundary gate, which requires
`git status --porcelain -- modules examples eclipse pom.xml src .github` to stay empty (§0.10.1);
editing `.github/dependabot.yml` would fail that gate on the way to satisfying a LOW finding. The CI
file is a separate surface with its own owner. The recommendations are recorded here so that whoever
owns repository governance can act on them as a governance change, which is what they are. Until one
of them is in place, the residual risk is reduced to a documented manual cadence rather than closed.

### Re-checking this section

```bash
grep -n "Dependabot\|Scala Steward\|advisory" SCALA_MIGRATION.md | head       # this section exists
grep -rniE "dependency review|advisory" README.md strata-collect/README.md \
    strata-basics/README.md                                                   # each README points here
git status --porcelain -- modules examples eclipse pom.xml src .github        # empty: .github untouched
```
