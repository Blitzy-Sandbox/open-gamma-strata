 ...
 *
 * The reference data table itself is asserted row for row against the data captured from the
 * implementation being ported, by `ReferenceDataManifestSpec`, and the closedness of every
 * named family is asserted by `NamedEnumClosedSpec`; neither is repeated here. This suite
 * asserts the behaviour of the type: how a code resolves, what a currency knows about itself,
 * how it rounds, orders, compares, renders and serializes. The cardinalities it does assert -
 * seventy-four rows, fifty-five available currencies, nine priority entries - are stated as the
 * shape of the family rather than as a transcription check, and they are the counts the
 * original itself relied on.
 *
...
final class CurrencySpec extends AnyFunSuite with Matchers with TableDrivenPropertyChecks {
  ...
  test("test_getAvailable") {
    ...
    // active rows are exactly the rows with constants is asserted by `test_consistency` below.
   ...
   * data has the shape the loader used to produce.
   */
  test("coverage") {
    ...
    (Hash[Currency].hash(gbp) == Hash[Currency].hash(eur)) shouldBe false
