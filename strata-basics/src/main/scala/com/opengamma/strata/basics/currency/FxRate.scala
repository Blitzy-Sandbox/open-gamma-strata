 ...
 * }}}
 *
 * The two constraints are those the implementation being ported declared in two different
 * places, and both are checked here in one expression so that a caller that breaks both is told
 * about both rather than about whichever happened to be tested first:
 *
 *   - the rate has to be greater than zero. Both signed zeros and every negative rate are
 *     rejected. A rate that is not a number is ''accepted'', because the check being ported
 ...
 *
 * ===Failure, where the original threw===
 *
 ...
 *
 * The equality synthesised for a case class compares the rate with the numeric comparison of the
 ...
 *   currency is exchanged for this amount of the counter currency. The bean being ported kept
 *   this accessor private and exposed the value only through `fxRate` and `toString`; here the
 *   accessor of the case class is public, which is a deliberate widening - the value is already
 *   observable through both of those members and through the `unapply` that pattern matching
 *   uses, so hiding it would remove nothing and would make the type awkward to pattern match on
...
sealed abstract case class FxRate private (pair: CurrencyPair, rate: Double)
   ...
   *
   * The reciprocal is computed as `1d / rate`, the expression the original used, so the result is
   * the same `Double` bit for bit rather than one that merely rounds to the same decimal.
   * Inverting twice therefore recovers a rate to within the rounding of two divisions and not
   ...
   * in terms of it. The four cases are tested in the order the original tested them: two
   * identical currencies convert at one whether or not this rate mentions them, this pair
   * answers with its rate, the inverted pair answers with the reciprocal, and any other pair has
   * no answer here.
   *
   * {{{
   ...
   * [[com.opengamma.strata.collect.result.Failure.CurrencyConversion]] carrying the message the
   * original threw with, naming both pairs. A cross that does exist can still fail, for the one
   * reason the product of two positive rates can be unusable: two rates small enough that their
   * product underflows to zero produce a rate `FxRate.of` rejects, which the original also
   * rejected from the constructor its cross called.
   *
   * @param other  the other rate
   * @return the rate derived from these two rates, or the failure describing why none exists
   */
   ...
   *
   * Another rate is equal when it holds the same pair and a rate with the same bit pattern. The
   * bit comparison is the one the generated bean performed, which differs from the comparison a
   * case class would have synthesised for exactly two rates: one that is not a number, which here
   * equals itself, and a negative zero, which here differs from a positive zero. An object of any
   ...
   * with the rate hashed by its bit pattern so that two rates which [[equals]] calls equal always
   * agree here too. The seed is the hash of the type's own name rather than the identity hash of
   * its class, which the generated bean used: that makes the hash of a rate a function of the
   * rate alone, identical in every run of every program, which is what the byte-stability
   ...
   * rather than `EUR/USD 5.0` - which is how the original wrote it and what documents and test
   * expectations carry. The three values outside the real numbers are written as the platform
   * writes them: `NaN`, `Infinity` and `-Infinity`.
   *
 ...
 * callers.
 *
 * ===What replaced the bean machinery===
 *
 * The type being ported was a generated immutable bean: it declared the positivity of the rate as
 * a property annotation, the identity constraint as a separate validator method, its text form
 * through conversion annotations, and its wire form reflectively. All four are ordinary code
 * here - two accumulating checks in `of`, the pattern and the two wordings of [[parse]], and
 * the two codecs at the end of this object, which are assembled at compile time and perform no
 * reflection. Nothing of the serialization the bean supported is kept: this object has no Java
 * serialization, no builder and no meta-bean.
 */
object FxRate {
   ...
   * space.
   *
   * This is the expression the implementation being ported matched, character for character, and
   * it is applied to the whole of the text rather than to part of it, so leading or trailing
   * characters are a rejection rather than something to ignore.
   *
   ...
   * That distinction decides which of the two wordings of [[parse]] a caller is given, so it is
   * kept exactly.
   */
   ...
   *
   * The wording is that of the validator being ported, word for word, because it reaches logs and
   * test expectations.
   */
   ...
   * @return the FX rate, or every reason the arguments do not describe one
   */
   ...
   * identity, in that order, in one failure chain. The implementation being ported checked them
   * in two different places and reported whichever it reached first.
   *
   * {{{
   ...
   *
   * Two wordings are reported, and which one a caller gets is decided exactly as it was in the
   * original. Text the expression above does not match is `Invalid rate: <text>`; text that
   * matches but does not name a rate - because a code names no currency, because the digits do
   * not form a number, or because the rate is one this type rejects - is
   * `Unable to parse rate: <text>`. Both quote the text as it was supplied rather than as it was
   * folded, so a caller sees back what they wrote.
   *
   * {{{
   ...
   * }}}
   *
   * One input is rejected here that the original accepted: text naming a currency outside the
   * closed set of this port, such as `EUR/ZZZ 1.25`. The original minted a currency for any
   * three-letter code, so it parsed; here [[Currency.parse]] rejects it and the text is reported
   * as naming no rate. That follows from the closed currency family of this port and is recorded
   * as a divergence with it.
   *
   * The cause of a rejection is deliberately not carried in the failure. The original wrapped it
   * in the exception it threw, but the message - which is what a caller reads, logs and asserts
   ...
   * currencies, because it is not equal to one - which is what the validator being ported
   * concluded from the same comparison.
   *
   * @param pair  the currency pair
   ...
   * [[CurrencyPair.cross]] has already chosen - in market convention order, which is why neither
   * input pair can be assumed to be the right way round. The orientation is therefore derived
   * rather than assumed, in the order the original derived it: which input holds the base
   * currency of the cross decides which rate is read towards the shared currency and which is
   * read away from it, and each rate is inverted if its own pair runs the other way.
   *
   * That derivation depends on the branch order of [[CurrencyPair.cross]], which is part of its
   * contract for this reason. The two multiplications and the two divisions are written in the
   * order the original wrote them, and the operands are not reassociated: floating-point
   * multiplication of a rate and a reciprocal is not associative, so a rearranged expression
   * would agree with the original only to within rounding, and the parity fixtures of this port
   * record the exact `Double`.
   *
   * @param fx1  the first rate
   * @param fx2  the second rate
   * @param crossPairAC  the cross pair, in market convention order
   * @return the cross rate, or every reason the product does not describe one
   */
   ...
   * this rendering does that the platform's own does not, and the reason the original reached for
   * a numeric helper of its collection library. That helper is replaced by
   * [[com.opengamma.strata.collect.DoubleArrayMath.isMathematicalInteger]], which answers the
   * same question with the same treatment of the three values outside the real numbers: none of
   * them is a whole number, so each is written as the platform writes it.
   *
   * A whole number beyond the range of a 64-bit integer is written as that range's largest
   * value, because the conversion saturates - a rate of `1e20` renders as
   * `9223372036854775807`. That is what the original rendered, from the same narrowing
   * conversion, and it is kept rather than corrected so that the text of a rate is unchanged by
   * this port.
   *
   ...
   *
   * No `Order` is offered. The type being ported was not comparable, and there is no ordering of
   * rates that means anything across pairs - a rate of `1.25` for `EUR/USD` is neither above nor
   * below a rate of `0.8` for `USD/JPY`. A caller that needs a reproducible sequence of rates
   * should sort by the pair, which [[CurrencyPair]] orders.
   *
  ...
  // else: the rate goes through the single policy this port has for a double, which writes the
  // values JSON cannot express as tagged strings. The import is what makes that choice
