 ...
 * range of the value, so there is nothing for a factory to reject. The primary constructor,
 * `apply` and `copy` are therefore all public, and the four named factories are named
 * alternatives to them, one per adjustment type.
 *
 * The two fields are `modifyingValue` and `type`, in that order. `type` is a reserved word of
 * this language and so is written in backticks wherever it appears, which changes how it is
 * spelled in source and nothing else: it remains the name of the field, the name of the
 * accessor and the key of the JSON form.
 *
 * ===Equality===
 *
 * Equality and hashing compare the modifying value by its bit pattern rather than by numeric
 * comparison. Two consequences are deliberate: a value that is not a number equals itself, so
 * an instance always equals itself, and a negative zero is distinct from a positive zero. The
 * type contributes its own equality, which is the identity of a closed set of four members.
 *
 * ===Thread safety===
 *
 ...
 * ===Serialization===
 *
 * An adjustment is written as JSON through the codec of its companion and in no other form. A
 * `case class` is `java.io.Serializable` whether or not the library wants it, so this type mixes
 * in [[NoJavaSerialization]]: a pair assembled by an object stream rather than by a constructor
 * or a factory of this type cannot be presented as an adjustment of this library.
 *
 * @param modifyingValue  the value that modifies the base value, which is given meaning by
 *   the associated type
...
final case class ValueAdjustment(modifyingValue: Double, `type`: ValueAdjustmentType)
   ...
   * base value minus 10%. The calculation is the one the type defines over the pair, in the
   * arithmetic shape [[ValueAdjustmentType]] writes it.
   *
   ...
   * that is undefined - is returned as it stands rather than reported, so a caller that
   * requires a finite result checks the value it gets back.
   *
   ...
   * not even equal to itself, so this definition is written in its place. An object of any
   ...
   *
   * The mixing is a seed, then each field in declaration order, with the modifying value
   * hashed by its bit pattern so that instances which `equals` calls equal always agree here
   * too. The seed is the hash of the type's own name rather than the identity hash of its
   * class, which makes the hash of an instance a function of the instance alone, identical in
   ...
   * per cent increase. The match is over the closed family of types, so every type has a
   * rendering here and no branch is reached by default.
   *
   * An adjustment that makes no change is rendered as `ValueAdjustment[result = input]`
   * rather than as an addition of zero. That form is selected by comparing the instance
   * against the `NONE` constant by ''value'', so a separately built zero delta amount -
   * `ofDeltaAmount(0)` - renders as `input` just as the constant itself does, and the
   * rendering is therefore stable across a round trip through JSON, where the decoded
   * adjustment is a distinct instance. A negative zero is distinct from `NONE` under the
   * bit-pattern equality above and so renders as an addition.
   *
 ...
 * There is deliberately no `Order`: a modifying value paired with a type has no ordering worth
 * inventing, since comparing the values of two adjustments of different types compares numbers
...
object ValueAdjustment {
   ...
   * choice of this constant. `toString` renders it as `ValueAdjustment[result = input]`.
   *
  ...
  /**
   * Obtains an instance whose result is the replacement value itself.
   *
   ...
   *
   * Renders what `toString` renders, which describes the calculation, so the two ways of
   ...
   * while the program runs. An instance encodes as an object holding its two fields,
   * `modifyingValue` first and `type` second:
   *
   * {{{
   * {"modifyingValue":20.0,"type":"DeltaAmount"}
   * }}}
   *
   * The modifying value is written through the shared double codec of this library, so a
