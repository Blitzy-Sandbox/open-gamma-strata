 ...
 *
 * Each row carries one FX index of the published reference data, with exactly the six columns that
 * data declares: the index name, the base currency, the counter currency, the fixing calendar, the
 ...
 * fields, because a pair is what an FX index is quoted against. The correspondence is exact and
 * mechanical - the `Base Currency` column is `currencyPair.base` and the `Counter Currency` column
 * is `currencyPair.counter` - so a row can be read back against the published data column for
 * column.
 *
 * ===Data, not behaviour===
 *
 * The row carries no derived value. In particular it does not carry the fixing date offset of an
 * index: there is no such column, and the offset follows from the maturity days and the two
 * calendars, so deriving it belongs with the index family in `Index.scala` rather than with this
 * transcription. For the same reason the maturity days and the maturity calendar are kept apart
 * here rather than composed into one days adjustment: composing them is the index family's work.
 *
 ...
 *                          the two currency columns of the published data
 ...
 *
 * This object carries the FX index table transcribed verbatim from the published reference data.
 * Nothing is located, parsed or cached at run time here: the sixteen rows below are Scala literals
 ...
 * more than that: the family is closed and mints no index for a currency pair it has no row for,
 * so this table alone fixes which pairs an index exists for. Adding or omitting a row therefore
 * does not merely change a lookup, it changes the published API, which is why no index may be
 * added to, removed from or edited in it.
 *
 * ===Invariants===
 *
 ...
 *
 *  - [[rows]] holds exactly sixteen entries with distinct names, in published order: the four
 *    European Central Bank rates, the four WM/Reuters rates, then the eight rates named after the
 *    industry definitions that publish them.
 ...
 *    Singaporean and Taiwanese ones - have no built-in calendar here, so those indices resolve
 *    against `ReferenceData.standard` with a missing data failure until a host supplies the
 *    calendar. Nothing is invented to fill the gap.
 ...
 *
 * There is likewise no alternate name table. This family accepts exactly one alternate name,
 * `USD/INR-RBIB-INR01` for the dollar/rupee rate named `USD/INR-FBIL-INR01` after the
 * administrator that publishes it, and an alternate name is a property of the family's lookup
...
private[basics] object FxIndexData {
  ...
  /**
   * The sixteen transcribed FX index rows, in published order.
   *
   ...
   * appear in published order too.
   *
   * A calendar that the library ships and names is written as its named constant, and one it does
   * not name is written as the total identifier factory applied to the text of the column. That
   * factory also builds the composite identifiers, and it normalises them: the parts are
   * deduplicated and sorted by name, so the column text `EUTA+CHZU` yields the identifier named
   * `CHZU+EUTA`. The literals below are the column text, and normalisation is the identifier's
   * business.
   */
   ...
   * presented in published order. That ordering is built here by folding
  ...
  /**
   * Builds one row from the six published columns.
   *
   * The base and counter currencies are separate parameters, so that the table above reads as the
   * published data does - six columns in their published order - while the row type holds the
   * single currency pair that the rest of the module works with. Pairing the two currencies here
   * keeps that step out of every row, and a pair of currencies is always meaningful, so this
   * cannot fail.
   *
   ...
   * on one line. Any name is accepted, whether or not this library ships the calendar it names,
   * and a name joining several calendars is normalised by the factory; see [[HolidayCalendarId.of]]
   * for both. An identifier naming a calendar that is not shipped is admitted here and fails to
   * resolve against reference data at the point of use, so a host that supplies the calendar makes
   * the index usable without a change to this table.
   *
