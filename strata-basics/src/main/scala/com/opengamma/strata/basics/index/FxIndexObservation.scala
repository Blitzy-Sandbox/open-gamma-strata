 ...
 *
 * There are exactly two ways into the type, and both of them derive the maturity date rather than
 * accepting it from a caller. [[of]] is the public one and resolves the calendars of the index on
 * every call, which is what a caller observing one fixing wants. [[create]] is the package-private
 * one and accepts a maturity date already computed, which is what a caller observing many fixings
 * of one index wants: [[FxIndex.resolveWith]] resolves the fixing calendar and the maturity
 * offset once and hands back a function over fixing dates, and composing that function with this
 * factory - `index.resolveWith(refData)(FxIndexObservation.create(index, _, _))` - produces
 * observations without resolving a calendar again. Keeping that second door package-private is
 * what stops a maturity date from being supplied by a caller who did not derive it.
 */
