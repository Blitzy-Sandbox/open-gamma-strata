...
sealed abstract case class Money private (currency: Currency, amount: Decimal)
  ...
  def plus(amountToAdd: Money): FailureOr[Money] =
    if (amountToAdd.currency == currency) {
      ...
      Left(Money.DifferentCurrenciesAddFailure)
    }
  ...
  def minus(amountToSubtract: Money): FailureOr[Money] =
    if (amountToSubtract.currency == currency) {
      ...
      Left(Money.DifferentCurrenciesSubtractFailure)
    }
...
object Money {
  ...
  /**
   * The failure [[Money.plus]] reports for a currency mismatch.
   *
  ...
  /** The failure [[Money.minus]] reports for a currency mismatch. */
  ...
  /** The failure [[Money.convertedTo]] reports for a rate other than one. */
   ...
   * because [[Money.parse]] locates the separator by scanning for it - a text whose first part
  ...
  /**
   * The longest text a failure of [[Money.parse]] quotes back in full.
   *
   ...
   * [[Money.parse]] was a thousand characters or a million. What a reader sees is unchanged,
   ...
   * is written out.
   *
   * @param text  the rejected text, as it was given to [[Money.parse]]
