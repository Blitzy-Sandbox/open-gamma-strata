...
sealed abstract case class OvernightIndexObservation private (
  ...
  // publication, effective and maturity dates that follow from no index - can be stopped is here.
  // The single implementation is the companion's hidden `Impl`.
  JvmClosure.requireSoleImplementation(this, classOf[OvernightIndexObservation.Impl])

  // The invariant of this type, stated over the fields the instance actually holds, because the
  // implementation class carries a public constructor whatever the source asked for and identity
  // alone would then admit an observation whose four derived values follow from nothing. The index
  // is a published member of its family - its own constructor established that - so its two
  // offsets and its day count are the true ones, and what is required here is what the derivation
  // of `resolve` implies without the reference data a constructor is never handed: each date lies
  // at or beyond the fixing date by the offset it is derived through, the two derived dates stand
  // in the order their offsets do, the borrowing lasts the one business day this family fixes, and
  // the year fraction is exactly the value the day count of the index measures over that period.
  JvmClosure.requireInvariant(
    "its publication and effective dates are no earlier than its fixing date advanced by the " +
      "offsets of its index",
    (index.publicationDateOffset < 0 ||
      !publicationDate.isBefore(fixingDate.plusDays(index.publicationDateOffset.toLong))) &&
      (index.effectiveDateOffset < 0 ||
        !effectiveDate.isBefore(fixingDate.plusDays(index.effectiveDateOffset.toLong))))
  JvmClosure.requireInvariant(
    "its publication and effective dates stand in the order the publication and effective " +
      "offsets of its index do",
    publicationDate.compareTo(effectiveDate).sign ==
      (index.publicationDateOffset - index.effectiveDateOffset).sign)
  JvmClosure.requireInvariant(
    "its maturity date is the business day after its effective date, and so later than it",
    maturityDate.isAfter(effectiveDate))
  JvmClosure.requireInvariant(
   ...
   * the one hidden subclass of it - the way a `sealed abstract case class` is instantiated - so its
...
object OvernightIndexObservation {
   ...
   * reachable only from inside the file that declares it, and `new Impl(...)` - the hidden
   * implementation class declared below - is how it is reached; that is what
  ...
  /**
   * The one implementation of an Overnight index observation.
   *
   ...
   * [[OvernightIndexObservation]] refuse in its own constructor to be any other implementation.
   *
   * @param index  the index observed
   * @param fixingDate  the date the rate is fixed on
   * @param publicationDate  the publication date derived from the fixing date
  ...
  private final class Impl(
      ...
      maturityDate: LocalDate,
      yearFraction: Double)
      extends OvernightIndexObservation(
        index,
        fixingDate,
        publicationDate,
        effectiveDate,
