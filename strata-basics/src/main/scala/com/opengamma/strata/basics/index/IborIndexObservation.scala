 ...
 *
 * That resolution lives here rather than on [[IborIndex]] itself, where the original declared it.
 * The index hierarchy of this port is one sealed file and states its per-fixing calculation over
 * the construction of a result rather than over this type, so that the hierarchy does not have to
 * name it; the function that names it belongs with the type it produces, which is here.
 */
